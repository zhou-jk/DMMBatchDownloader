...
from shared_utils import (
    ...
    CONFIG_FILE, DECRYPT_CONN_ERROR_MSG
)

# Configuration constants
RCLONE_REMOTE_NAME = "remote"  # Default remote name
REMOTE_DCV_PATH = "/root/jav-it"  # Remote DCV files path
REMOTE_MKV_PATH = "/root/jav-it/decrypted"  # Remote MKV files path

class RcloneDecryptProcessor:
    """Rclone DCV Decryption Processor Class"""
    
    def __init__(self, config: configparser.ConfigParser, remote_name: str = RCLONE_REMOTE_NAME):
        self.config = config
    ...
    def list_remote_dcv_files(self) -> List[str]:
        ...
        logging.info(f"Scanning remote path: {self.remote_name}:{REMOTE_DCV_PATH}")
        
        try:
            cmd = [
                'rclone', 'lsf', '--include', '*.dcv',
                f"{self.remote_name}:{REMOTE_DCV_PATH}"
            ]
    ...
    def download_dcv_file(self, filename: str, local_dir: str) -> Optional[str]:
        ...
        remote_file_path = f"{self.remote_name}:{REMOTE_DCV_PATH}/{filename}"
        local_file_path = Path(local_dir) / filename
        
        logging.info(f"Downloading: {filename}")
        
        try:
            cmd = ['rclone', 'copy', remote_file_path, str(local_dir), '-v']
    ...
    def upload_mkv_file(self, local_mkv_path: str, original_filename: str) -> bool:
        ...
        remote_mkv_path = f"{self.remote_name}:{REMOTE_MKV_PATH}"
        
        logging.info(f"Uploading decrypted file: {mkv_filename}")
        
        try:
            # Ensure remote directory exists
            mkdir_cmd = ['rclone', 'mkdir', remote_mkv_path]
            subprocess.run(mkdir_cmd, capture_output=True, timeout=30)
            
            # Upload file
            cmd = ['rclone', 'copy', local_mkv_path, remote_mkv_path, '-v']
...
def create_default_config():
    ...
    decrypt_executable = "jav-it.exe" if platform.system() == "Windows" else "jav-it"
    ...
    try:
        ...
        print(f"Failed to create configuration file {CONFIG_FILE}: {e}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser.add_argument('--remote', '-r', default=RCLONE_REMOTE_NAME,
                        help=f'Rclone remote name (default: {RCLONE_REMOTE_NAME})')
