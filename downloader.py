...
def download_dcv_file(download_url: str, output_dir: str, cid_part_label: str,
    ...
    if not response_r1 or not (300 <= response_r1.status_code < 400):
        ...
        logging.error(f"Redirect #1 failed for {cid_part_label}: {status}")
        return None
    
    url_r1 = response_r1.headers.get('Location')
    if not url_r1:
        logging.error(f"Redirect #1 for {cid_part_label}: no 'Location' header found.")
        return None
    logging.info(f"Redirect #1 for {cid_part_label}: {url_r1[:120]}...")
    ...
    if not response_r2 or not (300 <= response_r2.status_code < 400):
        ...
        logging.error(f"Redirect #2 failed for {cid_part_label}: {status}")
        return None
    
    final_url = response_r2.headers.get('Location')
    if not final_url:
        logging.error(f"Redirect #2 for {cid_part_label}: no 'Location' header found.")
        return None
